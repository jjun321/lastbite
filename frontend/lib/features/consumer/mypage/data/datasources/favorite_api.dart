      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        return data.map((json) {
          return StoreModel(
            ...
            storeAddress: json['store_address'] ?? json['address'] ?? '',
            storeLat: parseDouble(json['store_lat'] ?? json['latitude']),
            storeLon: parseDouble(json['store_long'] ?? json['longitude']),
            isClosed: json['is_closed'] ?? false,
            isOffToday: json['is_off_today'] ?? false,
            todayClose: json['today_close'] ?? json['closing_time'] ?? json['closingTime'] ?? '',
            isFavorite: true,
          );
        }).toList();
      }
