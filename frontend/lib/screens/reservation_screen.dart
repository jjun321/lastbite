...
class ReservationScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String getPickupTime() {
      ...
      String startTime = "${(min < 30 ? hour - 1 : hour)}:${(min < 30 ? min + 30 : min - 30).toString().padLeft(2, '0')}";
    ...
    int totalOriginalPrice = myCart.fold(0, (sum, item) => sum + (item.menu.originalPrice * item.quantity));
                ...
                child: Column(
                  ...
                  children: [
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  Text(myStore.name, style: const TextStyle(fontFamily: 'Sen', fontSize: 14, fontWeight: FontWeight.bold)),
                                  const Text('주문번호 #대기중', style: TextStyle(fontSize: 14, color: Color(0xFF6B6E82), decoration: TextDecoration.underline)),
                                ],
                              ),
                              const SizedBox(height: 6),
                              Row(
                                children: [
                                  Text('$totalQuantity개', style: const TextStyle(fontSize: 14, color: Color(0xFF181C2E), fontWeight: FontWeight.bold)),
                                  const Padding(
                                    padding: EdgeInsets.symmetric(horizontal: 8),
                                    child: Text('|', style: TextStyle(color: Color(0xFFCACCDA))),
                                  ),
                                  Text('$totalPrice원', style: const TextStyle(fontSize: 14, color: Color(0xFF6B6E82))),
                                ],
                              ),
                              const SizedBox(height: 4),
                              Text('픽업 ${getPickupTime()}', style: const TextStyle(fontSize: 13, color: Color(0xFF6B6E82))),
                            ],
                          ),
                        ),
                      ],
                    ...
                    Container(
                      ...
                      child: Column(
                        children: myCart.map((item) => Padding(
                          padding: const EdgeInsets.symmetric(vertical: 4),
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text("${item.menu.name} | ${item.quantity}개",
                                  style: const TextStyle(fontSize: 14, color: Color(0xFF181C2E))),
                              Text('${item.totalPrice}원', style: const TextStyle(fontSize: 14, color: Color(0xFF828282))),
                            ],
                          ),
                        )).toList(),
                      ),
                    ...
                    const Text('결제 금액', style: TextStyle(fontFamily: 'Sen', fontSize: 14, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 12),
                    _buildRow('정가', '$totalOriginalPrice원'),
                    _buildRow('할인액', '-$totalDiscount원'),
                    _buildRow('합계', '$totalPrice원', isBold: true, valueColor: Colors.black),

                    // --- 주문자 정보 ---
                    const SizedBox(height: 30),
                    const Divider(color: Color(0xFFEEF2F6)),
                    const SizedBox(height: 10),
                    const Text('주문자 정보', style: TextStyle(fontFamily: 'Sen', fontSize: 14, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 12),
              ...
              child: ElevatedButton(
                onPressed: () {
                  ...
                  final newOrder = Order(
                    orderNo: '#${DateTime.now().millisecondsSinceEpoch.toString().substring(8)}',
  ...
  Widget _buildRow(String label, String value, {bool isBold = false, Color labelColor = const Color(0xFF717171), Color valueColor = const Color(0xFF1E1E1E)}) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(value, style: TextStyle(fontSize: 14, fontWeight: isBold ? FontWeight.bold : FontWeight.normal, color: valueColor)),
        ],
      ),
    );
  }
